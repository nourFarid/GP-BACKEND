...
const addResult = errorHandling.asyncHandler(async(req,res,next)=>{
    try { 
        ...
        const updatedUser = await User.findOneAndUpdate({ NationalId }, { $set: { contextOfInquiry: contextOfInquiry } }, { new: true });
...
const getResultOfInquiry = errorHandling.asyncHandler(async (req, res, next) => {
  try {
      ...
      return res.status(200).json({ status: httpStatusText.SUCCESS, data: { context } });
  } catch (error) {
      // Handle errors here
      next(error);
  }
